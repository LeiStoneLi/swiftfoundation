...
let TGDropBoxListCellHeight: CGFloat = 25.0
...
//    fileprivate var spaceLine: UIView!
    ...
    fileprivate var backgroundImage: UIImageView! // under the box background
...
//       
//        self.boxButton.setImage(background, for: .normal)
        self.boxButton.layer.cornerRadius = 10
...
//        self.spaceLine.center = CGPoint(x: self.frame.size.width - 42.5, y: self.frame.size.height * 0.5)
        ...
        self.boxArrow.center = CGPoint(x: self.frame.size.width - 15, y: self.frame.size.height * 0.5)
...
//        let angle: CGFloat = ((self.isShow) ? -1 : 1)
        
        UIView.animate(withDuration: 0.2, animations: {
//            self.boxArrow.transform = self.boxArrow.transform.rotated(by: angle * CGFloat(M_PI/180))
            if !self.isShow {
                self.boxArrow.transform = CGAffineTransform(rotationAngle: CGFloat(M_PI))
            } else {
                self.boxArrow.transform = CGAffineTransform(rotationAngle: CGFloat(0))
            }
        })
